                ...
                <tbody class="divide-y divide-gray-200">
                    @foreach ($users as $user)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">{{ $user->addedBy->name ?? 'N/A' }}</td> <!-- Fetch the name of the user who added this user -->
                            <td class="px-6 py-4">
                                @foreach ($user->getRoleNames() as $role)
                                    <span class="inline-block bg-blue-100 text-blue-800 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded">{{ $role }}</span>
                                @endforeach
                            </td>
    ...
    $(document).ready(function() {
        $('#users-table').DataTable({
            ...
            "language": {
                ...
                "lengthMenu": "Display _MENU_ users per page", // Custom length menu
            },
            "columnDefs": [
